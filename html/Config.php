...
register_shutdown_function(static function () use ($phpErrorLog): void {
  ...
  $requestUri = $_SERVER['REQUEST_URI'] ?? 'CLI/unknown';
  $message = sprintf(
    "[PHP Config Fatal] %s | %s in %s:%d",
    $requestUri,
    $error['message'],
    $error['file'],
    $error['line']
...
if (isset($_POST['all_config_save'])) {
  ...
  $Config['smart_config']['smart_wakeup']['hotword']['ring_buffer_porcupine'] = intval($_POST['ring_buffer_porcupine']);
  $Config['smart_config']['smart_wakeup']['hotword']['ring_buffer_snowboy'] = intval($_POST['ring_buffer_snowboy']);
  ...
  $Config['xiaozhi']['pcm_output_gain'] = max(0.0, min(4.0, floatval($_POST['xiaozhi_pcm_output_gain'] ?? 1.0)));
  $oldXiaozhiOtaUrl = rtrim((string)($Config['xiaozhi']['system_options']['network']['ota_version_url'] ?? ''), '/') . '/';
  $newXiaozhiOtaUrl = rtrim(trim($_POST['xiaozhi_ota_version_url']), '/') . '/';
  $Config['xiaozhi']['system_options']['network']['ota_version_url'] = $newXiaozhiOtaUrl;
  if ($oldXiaozhiOtaUrl !== $newXiaozhiOtaUrl) {
            ...
            <div class="card accordion" id="accordion_button_streaming_server_audio">
              <div class="card-body">
                <h5 class="card-title accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_button_streaming_server_audio" aria-expanded="false" aria-controls="collapse_button_streaming_server_audio">
                  Streming Audio Server <font color=red> (VBot Client, Client - Server) </font><i class="bi bi-question-circle-fill" onclick="show_message('Kiểm Tra Và Test Hãy Truy Cập Vào Trang <b>Hướng Dẫn</b> Hoặc <a href=\'FAQ.php\' target=\'_blank\'>Nhấn Vào Đây</a>')"></i>:
				...
				<div class="alert alert-success" role="alert">
                  <div class="row mb-3">
					...
					<?php
					echo select_field(
						'streaming_server_connection_protocol',
						'Kiểu Loại Kết Nối <font color="red" size="6" title="Bắt Buộc Nhập">*</font>',
						[
							'socket' => 'Socket (Khuyến nghị, nên sử dụng)'
						],
						$Config['api']['streaming_server']['connection_protocol'],
						[]
					);
					?>
            ...
            <div class="card accordion" id="accordion_button_volume_setting">
              <div class="card-body">
                ...
                <div id="collapse_button_volume_setting" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#collapse_button_volume_setting">
				 ...
				 <div class="card">
                    <div class="card-body">
                      <h5 class="card-title" title="Âm Lượng (Volume)/Audio Out">Cài Đặt Mic &nbsp;<i class="bi bi-question-circle-fill" onclick="show_message('Bạn có thể tham khảo hướng dẫn tại đây: <a href=\'FAQ.php\' target=\'_blank\'>Hướng Dẫn</a> <br/> Lưu Ý: Nếu Bạn Sử Dụng Mic I2S: INMP441 kết hợp với MAX98357 Thì Cần Flash IMG (VBot I2S) Và Phải Đặt ID Mic Luôn Luôn Là (-1) Nhé')"></i> &nbsp;:</h5>
                      <div class="alert alert-primary" role="alert">
					  <?php
                      echo input_field('mic_id', 'ID Mic', $Config['smart_config']['mic']['id'] ?? -1, 'required', 'number', '', '', '', 'Bạn có thể tham khảo hướng dẫn tại đây: <a href="FAQ.php" target="_blank">Hướng Dẫn</a> <br/> Lưu Ý: Nếu Bạn Sử Dụng Mic I2S: INMP441 kết hợp với MAX98357 Thì Cần Flash IMG (VBot I2S) Và Phải Đặt ID Mic Luôn Luôn Là (-1) Nhé', 'border-success', 'Tìm Kiếm ID Mic', "scan_audio_devices('scan_mic')", 'btn btn-success border-success', 'onclick', '_blank');
                      ?>
                      <div class="row mb-3">
                  ...
                  <div class="card">
                    <div class="card-body">
                      <h5 class="card-title" title="Âm Lượng (Volume)/Audio Out">Âm Lượng (Volume)/Audio Out &nbsp;<i class="bi bi-question-circle-fill" onclick="show_message('<font color=green>- Trương trình sẽ tương tác và thay đổi âm lượng của trình phát VLC <br/>- Sẽ không can thiệp vào âm lượng trên hệ thống của thiết bị (Trương trình sẽ bị giới hạn mức âm lượng, nếu âm lượng của hệ thống alsamixer đầu ra bị hạn chế hoặc được đặt ở mức thấp)</font><br/>Bạn có thể tham khảo hướng dẫn tại đây: <a href=\'FAQ.php\' target=\'_blank\'> Hướng Dẫn</a>')"></i> &nbsp;:</h5>
                      <div class="alert alert-primary" role="alert">
					  <?php
            ...
            <div class="card accordion" id="accordion_button_hotword_engine">
              <div class="card-body">
                <h5 class="card-title accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_button_hotword_engine" aria-expanded="false" aria-controls="collapse_button_hotword_engine">
				  ...
				  <?php
					$options_hotword = ['porcupine' => '<font color=green>Picovoice/Procupine</font>', 'snowboy' => '<font color=green>Snowboy</font>', null => '<font color=red>Không Sử Dụng</font>'];
                ...
                <div id="collapse_button_hotword_engine" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordion_button_hotword_engine" style="">
                  <div class="alert alert-success" role="alert"> <div class="card">
                    <div class="card-body">
                      <h5 class="card-title">Hotword / Từ Nóng Đánh Thức <i class="bi bi-question-circle-fill" onclick="show_message('Danh sách file thư viện Porcupine: <a href=\'https://github.com/Picovoice/porcupine/tree/master/lib/common\' target=\'_blank\'>Github</a><br/>Mẫu các từ khóa đánh thức: <a href=\'https://github.com/Picovoice/porcupine/tree/master/resources\' target=\'_blank\'>Github</a>')"></i> :</h5>
					  ...
					  <div class="alert alert-success" role="alert">
						<h5 class="card-title">Khuếch Đại Âm Thanh Hotword WakeUp <i class="bi bi-question-circle-fill" onclick="show_message('Khi được bật sẽ kích hoạt Khuếch đại âm thanh đánh thức Hotword WakeUP Nên có giá trị từ 1.1 -> 2.5 (1 hoặc 1.0, hoặc dưới 1 sẽ không khuếch đại)')"></i> :</h5>
						<div class="row mb-3">
                        <label for="" class="col-sm-3 col-form-label">Kích Hoạt <i class="bi bi-question-circle-fill" onclick="show_message('Khuếch đại âm thanh đánh thức Hotword WakeUP Nên có giá trị từ 1.1 -> 2.5 (1 hoặc 1.0, hoặc dưới 1 sẽ không khuếch đại. Lớn hơn 3.0 có thể sẽ bị méo tiếng, gây nhiễu, nhận diện kém)')"></i> :</label>
                        <div class="col-sm-9">
                          <div class="form-switch">
                            <input class="form-check-input border-success" type="checkbox" name="hotword_gain_active" id="hotword_gain_active" <?php echo $Config['smart_config']['smart_wakeup']['hotword']['gain']['active'] ? 'checked' : ''; ?>>
                          </div>
                        </div>
                      </div>
                      <?php
					  echo input_field('hotword_gain', 'Mức Nhân Khuếch Đại (Gain)', $Config['smart_config']['smart_wakeup']['hotword']['gain']['value_float'] ?? 2.0, 'required', 'number', '0.1', '1.0', '', 'Khuếch đại âm thanh từ Mic để đánh thức Hotword Wakeup, Nên có giá trị từ 1.1 -> 2.5, nếu đặt là <b>1, 1.0, dưới 1</b> sẽ không khuếch đại', 'border-success', '', '', '', '', '');
					  ?>
						</div>
						<div class="alert alert-success" role="alert">
						<?php
						$value_select_wakeup = $Config['smart_config']['smart_wakeup']['hotword']['select_wakeup'];
						if ($value_select_wakeup === null) {$value_select_wakeup = '';}
						echo select_field('hotword_select_wakeup', 'Chọn Nguồn Đánh Thức', ['porcupine' => 'Picovoice/Porcupine (Nên Dùng)', 'snowboy' => 'Snowboy', '' => 'Không Sử Dụng Hotword WakeUP'], $value_select_wakeup, []);
						?>
					  ...
					  <?php
						echo input_field('ring_buffer_snowboy', 'Đệm âm thanh (Ring Buffer)', $Config['smart_config']['smart_wakeup']['hotword']['ring_buffer_snowboy'] ?? 5, 'required', 'number', '1', '5', '30', 'Tạo một hàng đợi vòng (ring buffer) để lưu các khung âm thanh (audio frame) đọc từ microphone, cung cấp dữ liệu cho bộ phát hiện wakeword. Hotword Snowboy nên để trong khoảng 15 –> 20 (Mặc định là 15)', 'border-success', '', '', '', '', '');
					  ?>
                        <center>
                        ...
                        <div class="card-body">
                          ...
                          $tts_list_select = [
                            ...
                            'tts_ggcloud' => [
                              ...
                              'help'  => "Hướng Dẫn Đăng Ký Hãy Xem Ở Hướng Dẫn Sau Trong Thư Mục <b>Guide</b> -> <b>Tạo STT Google Cloud</b><br/><br/>-Link: <a href='https://drive.google.com/drive/folders/1rB3P8rev2byxgRsXS7mAdkKRj7j0M4xZ' target='_blank'>https://drive.google.com/drive/folders/1rB3P8rev2byxgRsXS7mAdkKRj7j0M4xZ</a>",
                            ],
                            'tts_ggcloud_key' => [
                  ...
                  <div class="card accordion" id="accordion_button_cfg_gemini_assistant">
                    <div class="card-body">
                      <h5 class="card-title accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_button_cfg_gemini_assistant" aria-expanded="false" aria-controls="collapse_button_cfg_gemini_assistant">
                        Cấu Hình Trợ Lý => Google Gemini <i class="bi bi-question-circle-fill" onclick="show_message('Lấy Key/Api: <a href=\'https://aistudio.google.com/app/apikey\' target=\'_blank\'>https://aistudio.google.com/app/apikey</a> ')"></i>:
