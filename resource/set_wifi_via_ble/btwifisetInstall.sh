...
function getcountrycode() {
    # Auto set WiFi country code without prompting user
    local ctry=""
    if [ -f "$wpa" ]; then
        ctry=$($sudo grep -E "^country=" "$wpa" 2>/dev/null | head -n1 | cut -d= -f2)
    fi
    # Default country
    [ -z "$ctry" ] && ctry="VN"
    # Normalize
    ...
    country="${country^^}"
    # Validate
    if ! grep -q "^$country" /usr/share/zoneinfo/iso3166.tab; then
        echo "? Country '$country' invalid, fallback to VN"
        country="VN"
    fi
    echo "> WiFi Country code set to: $country"
}
...
echo "> Update APT source for Raspbian archive"
SRC_FILE="/etc/apt/sources.list"
OLD_SRC="deb http://raspbian.raspberrypi.org/raspbian/ bullseye main contrib non-free rpi"
NEW_SRC="deb http://archive.raspbian.org/raspbian bullseye main contrib non-free"
#Comment dòng cũ nếu tồn tại và chưa bị comment
if grep -q "^$OLD_SRC" "$SRC_FILE"; then
    echo "> Comment old raspbian source"
    $sudo sed -i "s|^$OLD_SRC|#$OLD_SRC|g" "$SRC_FILE"
fi
#Thêm dòng mới nếu chưa tồn tại
if ! grep -q "^$NEW_SRC" "$SRC_FILE"; then
    echo "> Add archive.raspbian.org source"
    echo "$NEW_SRC" | $sudo tee -a "$SRC_FILE" >/dev/null
fi
#Apply changes
echo "> Apply APT changes"
$sudo apt clean
$sudo apt update

echo "> Enable & start Bluetooth services"
...
echo "> Copy btwifiset files from local directory"
for f in btwifiset.py btpassword.py passwordREADME.txt
do
    if [ ! -f "$SCRIPT_DIR/$f" ]; then
        echo "? Missing file: $f in $SCRIPT_DIR"
        exit 1
    fi
    $sudo cp "$SCRIPT_DIR/$f" "$btwifidir/$f"
...
echo $"
Dịch vụ btwifiset đã được cài đặt thành công và có thể truy cập được thông qua ứng dụng BTBerryWifi, app có trên cả IOS và Android.
Hãy khởi động lại hệ thống khi cài đặt thành công bằng lệnh: 'sudo reboot' (điều này đảm bảo dịch vụ btwifiset khởi động đúng cách).
"
exit
