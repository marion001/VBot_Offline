    ...
    try:
        ...
        with open(LOG_FILE, "a", encoding="utf-8") as log_file:
            log_file.write(
                ...
                "============================================================\n"
                f"[{current_time}] "
                "[VBot Multiroom Controller] Khởi động thất bại\n"
                f"Lỗi: {error}\n"
...
def main():
    try:
        from VBot_Multiroom_Controller_Audio import main as controller_main
        return controller_main()
    except KeyboardInterrupt:
        return 0
    except Exception as error:
        current_time = datetime.now().strftime("%H:%M:%S %d-%m-%Y")
        print(f"[{current_time}] [VBot Multiroom Controller] Lỗi: {error}", file=sys.stderr)
